# 排错清单  /  Trouble-shooting checklist
...
**根因**（2026-01 验证，通过 `strings` 读 `libelmersolver.dll` 字符串）：
...
Elmer 26.1 用 `<exepath>` 拼相对路径找 `share\elmersolver`。`%s` 是
`exepath`，从 `GetModuleFileNameW(NULL, ...)` 拿，但 **PowerShell
`&` 或 `Start-Process` 启动 native exe 时 `exepath` 解析失败**，
导致 `<exepath>/../share/elmersolver` 变成空路径，procedure DLL
就找不到。

**已尝试且不稳定的 workaround**：

* `Set-Location "D:\Program Files\Elmer 26.1-Release"; & .\bin\ElmerSolver.exe <sif>` — 偶发可行
* `Start-Process -WorkingDirectory "D:\..."` — 失败
* `ELMER_HOME` / `ELMER_LIB` env vars — 只对 `SOLVER.KEYWORDS` 和 `elements.def` 有效，**对 procedure DLL 无效**
* 拷贝 `MagnetoDynamics.dll` 到工作目录 — 失败
* 建 `share\elmersolver\lib` junction — 失败
* 创建 `loadall.dll` 占位 — 未测试

**建议**：

* 升级到 Elmer >= 26.2（如果已发布）或回退到 25.x
* 或：直接在 cmd.exe 手动跑：
...
* 体积分类用 bounding-box 而不是 centre-of-mass（fragment 后
  几何中心可能跑到外部）。
...
* `AV` 是 base Elmer scalar 关键字（1 分量），不是矢量形式。
...
* 自由下落时间预算：磁铁初始 z 中心 75 mm，到绕组底面 z=0 需时
  `sqrt(2 * 0.075 / 9.81) ≈ 0.124 s`。220 步 × 8e-5 s = 17.6 ms，磁铁
  还来不及走一半；要做完整 0.124 s 仿真需要 ~1500 步。
